...
# Create a vector of ISO3 codes for the countries of interest.
iso3_available <- c("USA", "CHN", "DEU", "TUR")

# Function to determine the highest administrative level available from GADM.
get_max_admin_level <- function(iso3) {
    # Attempt to download at a relatively high level (here, level 3) without error.
    # For countries with fewer levels, GADM will return only the available ones.
    gadm_data <- try(gadm(iso3, level = 3, path = localfolder, overwrite = TRUE), silent = TRUE)
    if (inherits(gadm_data, "try-error")) {
        # If the download fails at level 3, try a lower level (e.g., level = 2)
        gadm_data <- gadm(iso3, level = 2, path = localfolder, overwrite = TRUE)
    }
    # Identify columns for administrative IDs (e.g., "GID_0", "GID_1", "GID_2", ...)
    gid_cols <- grep("^GID_", names(gadm_data), value = TRUE)
    # Extract the numeric portions and determine the maximum level
    max(as.numeric(sub("GID_", "", gid_cols)))
}

# Create a data frame with the ISO3 codes and their highest available admin level.
gadm_info <- data.frame(
    iso3 = iso3_available,
    max_level = sapply(iso3_available, get_max_admin_level)
)
print(gadm_info)
